...
Review labels show the number of matching entries when greater than zero.
Empty views keep their original label without a count. All shortcuts still
work, including commands that open empty views.
Counts appear for someday/maybe, upcoming delegated items, completed projects,
and the stuck and missed sub-menus. Area-of-focus and completed-item reviews
have no count because their results depend on user input.

Counts use the review's native Org matcher and skip function. Project counts
refer to project headings, not their tasks. Multi-block totals count each
matching entry in each block. For example, a project with both an overdue
deadline and an overdue scheduled date appears twice in the missed-projects
total. Counts describe matches before display-only agenda filters or limits.
They follow the view's TODO, timestamp, inheritance, archive and comment rules.

Opening a menu computes one snapshot from current agenda sources, including
unsaved edits. The snapshot stays fixed while that menu is open. Close and reopen
the menu to refresh it after edits, configuration changes or a date change.
Opening a sub-menu computes a new snapshot for that sub-menu.

Counts do not create an agenda window or save files. Project checks also read
Org's ID sources into a private index. Each distinct view is counted once per
opening, with no scans during redisplay. For large or remote collections, set
~org-gtd-review-show-counts~ to ~nil~ to omit counts and these scans.
If counting fails, the menu shows ~(?)~ and reports a warning. Review commands
remain available.
